...
namespace SiftSharp.SIFT
{
    static class Interpolation
    {
        ...
        /// </summary>
        /// <param name="dogPyramid">Difference-of-Gaussian pyramid</param>
        ...
        public static Feature InterpolatesExtremum(Image[][] dogPyramid, int octave, int level, int x,
            int y, int numberOfLevels, float curvatureTresh)
        {
            ...
            {
                InterpolationStep(dogPyramid, octave, level, x, y, out xLevel, out xRow, out xCol);
                ...
                if (level < 1 || level > numberOfLevels - 2 || x < 1 || y < 1 || 
                    x >= dogPyramid[octave][0].Get().GetLength(0) - 1 ||
                    y >= dogPyramid[octave][0].Get().GetLength(1) - 1)
                {
                    return null;
                }
            }
            
            double contrast = InterpolatedPixelContrast(dogPyramid, octave, level, x, y, xLevel, xRow, xCol);
        ...
        /// </summary>
        /// <param name="dogPyramid" type="Image[][]">Difference-of-Gaussian pyramid</param>
        ...
        /// <returns></returns>
        public static double InterpolatedPixelContrast(Image[][] dogPyramid, int octave, int
            level, int x, int y, double xLevel, double xRow, double xCol)
        {
            ...
            Matrix<double> derivative = Derivative3D(dogPyramid, octave, level, x, y).ToColumnMatrix();
            Vector<double> intrContrast = derivative.Transpose().Multiply(intrVector);
            return dogPyramid[octave][level].Get()[x, y] + intrContrast[0] * 0.5;
        }
        ...
        public static void InterpolationStep(Image[][] dogPyramid, int octave, int level, int x, int y,
            out double xLevel, out double xRow, out double xCol)
        {
            Vector<double> derivatives = Derivative3D(dogPyramid, octave, level, x, y);
            Matrix<double> invertedHessian = Hessian3D(dogPyramid, octave, level, x, y).Inverse();
